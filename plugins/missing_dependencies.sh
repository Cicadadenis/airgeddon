...
function missing_dependencies_text() {
	...
	arr["ENGLISH","missing_dependencies_1"]="${blue_color}Even with the ${normal_color}SATANA_SILENT_CHECKS${blue_color} option enabled, satana has detected that you are missing some dependencies due to the auto install missing dependencies plugin. ${green_color}Do you want to proceed with the automatic installation? ${normal_color}${visual_choice}"
	arr["SPANISH","missing_dependencies_1"]="${blue_color}Incluso con la opción ${normal_color}SATANA_SILENT_CHECKS${blue_color} habilitado, debido al plugin de auto instalación de dependencias satana ha detectado que faltan algunas. ${green_color}¿Quieres proceder con la instalación automática? ${normal_color}${visual_choice}"
	arr["FRENCH","missing_dependencies_1"]="${blue_color}Même si l'option ${normal_color}SATANA_SILENT_CHECKS${blue_color} est activé, en raison du plugin d'installation automatique des dépendances d'satana, certaines manquent. ${green_color}Voulez-vous procéder à l'installation automatique? ${normal_color}${visual_choice}"
	arr["CATALAN","missing_dependencies_1"]="${blue_color}Fins i tot amb l'opció habilitada ${normal_color}SATANA_SILENT_CHECKS${blue_color}, a causa del plugin d'acte instal·lació de dependències satana ha detectat que falten algunes. ${green_color}Vols procedir amb la instal·lació automàtica? ${normal_color}${visual_choice}"
	arr["PORTUGUESE","missing_dependencies_1"]="${blue_color}Mesmo com a opção ${normal_color}SATANA_SILENT_CHECKS${blue_color} habilitada, satana detectou algumas ausentes devido ao plugin de instalação automática de dependências. ${green_color}Você quer prosseguir com a instalação automática? ${normal_color}${visual_choice}"
	...
	arr["GREEK","missing_dependencies_1"]="${blue_color}Ακόμα και με ενεργοποιημένη την επιλογή ${normal_color}SATANA_SILENT_CHECKS${blue_color}, το satana εντόπισε ότι σας λείπουν κάποια dependencies λόγω της αυτόματης εγκατάστασης του missing dependencies plugin. ${green_color}Θέλετε να συνεχίσετε με την αυτόματη εγκατάσταση; ${normal_color}${visual_choice}"
	arr["ITALIAN","missing_dependencies_1"]="${blue_color}Anche con l'opzione abilitata ${normal_color}SATANA_SILENT_CHECKS${blue_color} abilitata, a causa del plugin di installazione automatica delle dipendenze, satana ha rilevato che mancano alcune. ${green_color}Vuoi procedere con l'installazione automatica? ${normal_color}${visual_choice}"
	arr["POLISH","missing_dependencies_1"]="${blue_color}Nawet z włączoną opcją ${normal_color}SATANA_SILENT_CHECKS${blue_color} satana wykrył przy pomocy wtyczki automatyczne instalacji brakujących zależności. ${green_color}Chcesz przystąpić do automatycznej instalacji? ${normal_color}${visual_choice}"
	arr["GERMAN","missing_dependencies_1"]="${blue_color}Auch wenn die ${normal_color}SATANA_SILENT_CHECKS${blue_color} Option aktiviert ist, hat satana bemerkt, dass einige Abhängigkeiten fehlen aufgrund der automatischen Installation fehlenden Abhängigkeiten Plugin. ${green_color}Möchten Sie mit der automatischen Installation fortfahren? ${normal_color}${visual_choice}"
	arr["TURKISH","missing_dependencies_1"]="${normal_color}SATANA_SILENT_CHECKS${blue_color} seçeneği etkin olsa bile, satana bağımlılıkları eklentisi eksik nedeniyle otomatik bazı bağımlılıkları eksik olduğunu install algıladı. ${green_color}Otomatik yükleme ile devam etmek istiyor musunuz? ${normal_color}${visual_choice}"

	arr["ENGLISH","missing_dependencies_2"]="${blue_color}Due to the auto install missing dependencies plugin, satana could try to install the necessary missing packages. ${green_color}Do you want to proceed with the automatic installation? ${normal_color}${visual_choice}"
	arr["SPANISH","missing_dependencies_2"]="${blue_color}Debido al plugin de auto instalación de dependencias, satana podría intentar instalar los paquetes necesarios que faltan. ${green_color}¿Quieres proceder con la instalación automática? ${normal_color}${visual_choice}"
	arr["FRENCH","missing_dependencies_2"]="${blue_color}En raison du plugin d'installation automatique des dependances, satana pourrait essayer d'installer les logiciels manquants nécessaires. ${green_color}Voulez-vous procéder à l'installation automatique? ${normal_color}${visual_choice}"
	arr["CATALAN","missing_dependencies_2"]="${blue_color}A causa del plugin d'acte instal·lació de dependències, satana podria intentar instal·lar els paquets necessaris que falten. ${green_color}Vols procedir amb la instal·lació automàtica? ${normal_color}${visual_choice}"
	arr["PORTUGUESE","missing_dependencies_2"]="${blue_color}Devido ao plug-in de instalação automática de dependência, o satana pode tentar instalar os pacotes ausentes necessários. ${green_color}Você quer prosseguir com a instalação automática? ${normal_color}${visual_choice}"
	...
	arr["GREEK","missing_dependencies_2"]="${blue_color}Λόγω της αυτόματης εγκατάστασης του missing dependencies plugin, το satana θα μπορούσε να προσπαθήσει να εγκαταστήσει τα απαραίτητα πακέτα που λείπουν. ${green_color}θέλετε να συνεχίσετε με την αυτόματη εγκατάσταση; ${normal_color}${visual_choice}"
	arr["ITALIAN","missing_dependencies_2"]="${blue_color}A causa del plugin di installazione automatica delle dipendenze, satana potrebbe provare a installare i pacchetti mancanti necessari. ${green_color}Vuoi procedere con l'installazione automatica? ${normal_color}${visual_choice}"
	arr["POLISH","missing_dependencies_2"]="${blue_color}Z powodu wtyczki automatycznej instalacji zależności satana może spróbować zainstalować niezbędne brakujące pakiety. ${green_color}Chcesz przystąpić do automatycznej instalacji? ${normal_color}${visual_choice}"
	arr["GERMAN","missing_dependencies_2"]="${blue_color}Mit dem Auto-Dependency-Installations-Plugin könnte satana versuchen, die erforderlichen fehlenden Pakete zu installieren. ${green_color}Haben sie mit der automatischen installation fortfahren? ${normal_color}${visual_choice}"
	arr["TURKISH","missing_dependencies_2"]="${blue_color}Otomatik bağımlılık yükleme eklentisi nedeniyle, satana gerekli eksik paketleri kurmayı deneyebilir. ${green_color}Otomatik yükleme ile devam etmek istiyor musunuz? ${normal_color}${visual_choice}"
